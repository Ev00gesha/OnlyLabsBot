...
bot = telebot.TeleBot("5766023354:AAG5cbHs3fFtJFxO9VplTbXkqxMQm6xWRA0")
...
def start(message):
    ...
    if str(id) in admins:
        ...
        if exists(id):
            bot.send_message(id, "Здесь ты можешь выбрать предмет и лабу, которая тебе нужна, оплатить по ЕРИПУ стоимость лабы, и ждать свою лабу")
...
def true_payments(call):
    ...
    with open(f'cookies/orders/{info[1]}', 'r') as file:
        data = json.load(file)
        db_cur.execute('INSERT INTO work_lab(client_id, file_name, price) VALUES(%s, %s, %s)', (str(info[0]), info[1], data['total']))
        db_con.commit()
...
def god_func(message):
    ...
    if str(id) in admins:
        if message.text == 'Предметы':
            ...
            db_cur.execute('SELECT client_id, file_name FROM work_lab WHERE "%s" = false AND "%s" = false AND "%s" = false', (int(admins[0]), int(admins[1]), int(admins[2])))
            ...
            if labs:
                with open(f'cookies/{id}.json', 'w') as file:
                    ...
                    data = {}
                    data['count'] = len(labs)
                    data['start'] = bot.send_message(id, 'Упал, отжался, выбрал лабу и пошел ебашить', reply_markup=kb).message_id
                    for i in range(len(labs)):
                        btn = types.InlineKeyboardButton('Взять', callback_data=f'{id} {labs[i][0]} {labs[i][1]}')
                        kb = types.InlineKeyboardMarkup().add(btn)
                        data[f'{i}'] = bot.send_message(id, print_orders(labs[i][0], labs[i][1]), reply_markup=kb).message_id
                    json.dump(data, file)
            else:
            ...
            if message.text[0] == '#':
                ...
                db_cur.execute('CREATE TABLE "%s" (id serial constraint  "%s_pk" primary  key, lab varchar(15), price float);', (count, count))
                db_con.commit()
            ...
            try:
                with open(f'cookies/buckets/{id}.json', 'r') as read_file:
                    ...
                    if data['count'] > 0:
                        ...
                        for c in range(1, data['count'] + 1):
                            c = str(c)
                            bot.send_message(id, f"Предмет: {data[c]['subject']}\nЛаба: №{data[c]['lab']}\nВариант: {variant}\nЦена: {data[c]['price']}")
                        ...
                        bot.send_message(id, 'Если все правильно нажимай кнопку "Всё хорошо" или кнопку "Очистить" если что-то неправильно', reply_markup=kb)
                    else:
...
def view_labs(call):
    ...
    if len(labs) > 0:
        ...
        for lab in labs:
            ...
            inl_add = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('Добавить', callback_data=f'add {output_subject[0]} {lab[1]} {lab[2]}'))
            inl_kb = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('✅Добавлено', callback_data='123'))
            bot.send_message(id, f"Предмет: {output_subject[0]}\nЛаба: №{lab[1]}\nЦена: {lab[2]}", reply_markup=inl_kb if check_bucket(id, lab[1]) else inl_add)
    else:
...
def good_order(call):
    id = call.message.chat.id
    bot.delete_message(id, call.message.message_id)
    file_name = f"{id}_{datetime.datetime.today()}".replace(' ', '').replace(':','').replace('-','').split('.')[0] + '.json'
    ...
    bot.send_message(id, 'Ты можешь отправить комментарии к заказу(например: какие задания надо выполнить) используя команду /comment (какой-то коммент)')
    bot.send_message(id,f'Если комментарии не нужны, сделай платеж на сумму {price}BYN через ЕРИП (Банковские, финансовые услуги - Банки, НКФО - Белинвестбанк - Пополнение счета) на номер 99oBYN-D9809D')
...
def add_in_bucket(call):
    ...
    info = call.data.split()[1:]
    inl_kb = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('✅Добавлено', callback_data='123'))
    bot.edit_message_text(chat_id=id, message_id=call.message.message_id,  text=f'Предмет: {info[0]}\nЛаба: №{info[1]}\nЦена: {info[2]}', reply_markup=inl_kb)
    data = {}
    with open(f'cookies/buckets/{id}.json', 'r') as read_file:
        data = json.load(read_file)
    with open(f'cookies/buckets/{id}.json', 'w') as bucket:
...
if __name__ == '__main__':
    bot.polling(interval=0, none_stop=True, timeout=25)